...
pub fn main() {
    ...
    for i in 0..messages.len() {
        let m = messages[i];
        let s1_s2_g1 = (blob.c.1 - m.0).into_affine();
        let s2_g1 = blob.rec_pk;
        let hash_c = blob.c.hash_to_curve();
        let hash_c_s1 = blob.s;
        let lhs = { Bls12_381::pairing(s1_s2_g1, hash_c) };
        let rhs = { Bls12_381::pairing(s2_g1, hash_c_s1) };
        if lhs == rhs {
            println!("Found the message: {}", i);
            break;
        }
    }
    // answer: 3
